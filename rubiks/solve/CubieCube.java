...
class CubieCube {
	...
	/**
	 * Notice that Edge Perm Coordnate and Corner Perm Coordnate are the same
	 * symmetry structure. So their ClassIndexToRepresentantArray are the same. And
	 * when x is RawEdgePermCoordnate, y*16+k is SymEdgePermCoordnate,
	 * y*16+(k^e2c[k]) will be the SymCornerPermCoordnate of the State whose
	 * RawCornerPermCoordnate is x.
	 */
	...
	/**
	 * Check a cubiecube for solvability. Return the error code. 0: Cube is solvable
	 * -2: Not all 12 edges exist exactly once -3: Flip error: One edge has to be
	 * flipped -4: Not all corners exist exactly once -5: Twist error: One corner
	 * has to be twisted -6: Parity error: Two corners or two edges have to be
	 * exchanged
	 */
	int verify() {
		int sum = 0;
